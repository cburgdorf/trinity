...
class TrackerRunnerPlugin(BaseMainProcessPlugin):
    """
    Run Trinity a specified amount of times with performance tracking enabled.
    Archive the current database between each run.
    """
...
class PerformanceTrackerPlugin(BaseIsolatedPlugin):
    """
    Track the time from the start of Trinity to the successful sync to a specified block.
    """
    ...
    def do_start(self) -> None:
        ...
        try:
            # This is temporary to price in the effects of
            # https://github.com/ethereum/trinity/pull/173
            chain_db = self.boot_info.trinity_config.get_app_config(  # type: ignore
                Eth1AppConfig).readonly_chaindb
            self.logger.info("Using readonly chaindb (rocksdb only)")
        except AttributeError:
            chain_db = create_db_consumer_manager(  # type: ignore
                self.boot_info.trinity_config.database_ipc_path).get_chaindb()
            self.logger.info("Using proxied chaindb")
