...
class RequestServerPlugin(BaseIsolatedPlugin):
    ...
    def should_receive_broadcast(event: BaseEvent) -> bool:
        ...
        if isinstance(event, PeerPoolMessageEvent):
            # FIXME: Should not be hardcoded to ETHIsolatedRequestServer
            return type(event.cmd) in ETHIsolatedRequestServer._handled_commands

        return BaseIsolatedPlugin.should_receive_broadcast(event)
    ...
    def do_start(self) -> None:
        ...
        if eth1_app_config.database_mode is Eth1DbMode.LIGHT:
            ...
            les_peer_pool = LESProxyPeerPool(self.event_bus, TO_NETWORKING_BROADCAST_CONFIG)
            ...
            eth_peer_pool = ETHProxyPeerPool(self.event_bus, TO_NETWORKING_BROADCAST_CONFIG)
