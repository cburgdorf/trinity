...
async def make_peer_pool_answer_event_bus_requests(event_bus, peer_pool, handler_type=None):
    ...
    handler_type = ETHPeerPoolEventServer if handler_type is None else handler_type
...
async def run_isolated_request_server(proxy_peer_pool, chaindb, server_type=None):
    server_type = ETHIsolatedRequestServer if server_type is None else server_type
...
async def run_proxy_peer_pool(event_bus):
